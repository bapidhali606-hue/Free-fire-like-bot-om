...
# Public-group mode: this list is retained only for legacy admin commands.
ALLOWED_GROUP_IDS = []  # Deprecated: public-group mode ignores this whitelist

REPORT_CHAT_ID = -1003894818072

API_URL = "https://two0likeapifreebyzexxyh4x.onrender.com/like?key=20LikeFreeApiByzexxyh4x&uid={uid}&region={region}"
DEMO_API_URL = "https://two0likeapifreebyzexxyh4x.onrender.com/like?uid={uid}&region={region}&key=20LikeFreeApiByzexxyh4x"
...
async def group_only(update: Update):
    """PUBLIC GROUP MODE: no fixed group ID/whitelist is required.
    Owner-only commands still enforce OWNER_IDS separately.
    """
    ...
    if chat.type in ("group", "supergroup"):
        print(f"📌 PUBLIC GROUP: {chat.id} ({chat.title})")
        return True

    # Keep /start and /help available in private chat.
    if chat.type == "private":
        ...
        if update.message:
            await update.message.reply_text(
                premium_result("PUBLIC GROUP MODE", [
                    "🌐 This bot works in public groups.",
                    "➕ Add the bot to your group and use the commands there.",
                    "👑 Owner controls are restricted to OWNER_IDS."
                ], ok=False)
            )
        return False
...
async def auto_leave(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Public-group mode: never auto-leave a newly added group."""
    try:
        if not update.my_chat_member:
            return
        chat = update.effective_chat
        if chat and chat.type in ["group", "supergroup"]:
            print(f"✅ Public group accepted: {chat.id} ({chat.title})")
    except Exception as e:
        print(f"⚠️ Group event error: {e}")
...
def premium_loading(title="PROCESSING"):
    return (
        f"{PREMIUM_HEADER}\n"
        f"║  ⚡ 𝐙𝐗 𝐎𝐌 𝐅𝐅 // {title}\n"
...
def premium_result(title, lines, ok=True):
    icon = "🟢" if ok else "🔴"
    body = "\n".join(f"║  {line}" for line in lines)
    return (
        f"{PREMIUM_HEADER}\n"
        f"║  {icon} 𝐙𝐗 𝐎𝐌 𝐅𝐅 // {title}\n"
        f"╠══════════════════════════════╣\n"
        f"{body}\n"
        f"╠══════════════════════════════╣\n"
        f"║  ⚡ 𝐁𝐘:~ 𝐙𝐗 𝐎𝐌 𝐅𝐅           ║\n"
...
async def call_api(session, uid, region, api_url=None):
    ...
    if api_url is None:
        api_url = API_URL
    try:
        async with session.get(api_url.format(uid=uid, region=region), timeout=18) as r:
            return await r.json()
    except Exception as e:
        print(f"API Error: {e}")
        return None
...
async def send_like(app, session, entry, remaining):
    ...
    if likes_given > 0:
        ...
        msg = (
            ...
            f"Current: {after}\n"
            f"Status: {status}\n\n"
            f"👑 ZX LIKE BOT (@ZX OM FF)"
        )

    keyboard = [
        [
            InlineKeyboardButton("💰 Buy Autolike", url="https://t.me/zx_chat_gurop"),
            InlineKeyboardButton("📢 Join Channel", url="https://t.me/zx_chat_gurop")
        ]
    ]
...
async def perform_auto_like(app, manual=False):
    ...
    keyboard = [
        [
            InlineKeyboardButton("💰 Buy Autolike", url="https://t.me/zx_chat_gurop"),
            InlineKeyboardButton("📢 Join Group", url="https://t.me/zx_chat_gurop")
        ]
    ]
...
async def runall_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(premium_loading("AUTO-LIKE ENGINE"))
...
async def autolike_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    wait_msg = await update.message.reply_text(premium_loading("UID VERIFICATION"))
    ...
    keyboard = [
        [
            InlineKeyboardButton("💰 Buy Autolike", url="https://t.me/𝐙𝐗_𝐎𝐌_𝐅𝐅"),
            InlineKeyboardButton("📢 Join Channel", url="https://t.me/zx_chat_gurop")
        ]
    ]
...
async def like_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin_user:
        try:
            ...
            if not (ok1 and ok2):
                keyboard = [
                    [InlineKeyboardButton("📢 Join Channel 1", url="https://t.me/zx_chat_gurop")],
                    [InlineKeyboardButton("📢 Join Channel 2", url="https://t.me/zx_chat_gurop")]
                ]
    ...
    wait_msg = await update.message.reply_text(premium_loading("API SCAN"))
    ...
    if likes_given == 0:
        msg = (
            f"⚠️🚫 𝐋𝐈𝐊𝐄 𝐋𝐈𝐌𝐈𝐓 𝐑𝐄𝐀𝐂𝐇𝐄𝐃 🚫⚠️\n"
            f"────────────────────\n"
            f"👤 Player: {name}\n"
            f"🆔 UID: {uid}\n"
            f"🌍 Region: {region.upper()}\n\n"
            f"❤️ Current Likes: {after}\n"
            f"📡 Status: {status}\n\n"
        ...
        msg = (
            ...
            f"🌍 Region: {region.upper()}\n\n"
            f"🎯 Target Likes: 150\n"
            f"✅ Likes Added By Bot: {likes_given}\n"
            f"📉 Failed to Send: 0\n\n"
            f"📊 Before Like: {before}\n"
            f"📈 After Like: {after}\n\n"
            f"🔄 Remain API Limit: ♾️ Unlimited\n\n"
            f"👑 𝐁𝐘:~ 𝐙𝐗 𝐎𝐌 𝐅𝐅\n"
            f"📢 @𝐙𝐗_𝐎𝐌_𝐅𝐅"
        )
...
"""
    ...
    keyboard = [
        [InlineKeyboardButton("📢 Join Channel", url="https://t.me/zx_chat_gurop")],
        [InlineKeyboardButton("💰 Buy Autolike", url="https://t.me/𝐙𝐗_𝐎𝐌_𝐅𝐅")],
        [InlineKeyboardButton("👑 Owner Contact", url="https://t.me/ZX_OM_FF_01")]
    ]
